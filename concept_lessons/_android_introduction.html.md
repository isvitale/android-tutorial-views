...
Android has come a long since its inception in 2009 - A lot of things have changed from Cupcake to KitKat. We have not only seen great Android Phones but a mature operating system and a beautiful App experience.

Though a lot has changed since the launch of Android, the basic building blocks of the Android app have remained the same. Let's look at some of the basic building blocks of an Android app in detail. Later, we will see how all these components come together to create an app, using the example of a Twitter application.

An Android Application is coded in Java and compiled into a single distribution package called an APK. APK is the executable file which is installed on your phone as an Android app. Let's walk through the various building blocks of an Android application.
...
* So what exactly is an Activity ? Every screen in an Android app is an Activity.
Activity's will always have a User Interface (abbreviated as UI). It is not possible to have an Activity without a UI.
...
This is usually referred to as the launcher Activity or the Main Activity. 
...
* Every Android app has a Manifest.xml where all Activites are defined and one Activity is marked as the Main Activity. One of the most common errors developers commit when they start with Android development is forgetting to add a new Activity in Mainfest.xml file.
...
User Interface or UI is what the user sees on the screen. The Activity has the responsibility to *set* the UI for the screen. The UI is comprised primarily of two types of sub-components: **Views & Layouts** or **ViewGroups**. 

View as the name suggest is a basic building block like buttons, labels, input-box etc. Layout is the container for View elements. The Layout primarily defines the pattern in which the View element should show up. Eg. LinearLayout mandates that the View elements inside it either stack up horizontally or vertically while RelativeLayout lets each View element position itself relative to the parent or a sibling element. 
...
This is achieved using Services. Services is the Android way of keeping an operation going in the background.  When you need to have long running tasks like playing music, downloading data or uploading photos, it is achieved through Service.
 
Service doesn’t have a UI. To show any information to the user from Service, Notifications are used. 
...
*    Notifications are simple messages which are used to display information to the user that doesn’t require the user’s immediate attention 
*    Typically notifications come with text and a small icon, but newer versions of Android support richer notifications with buttons and images
*    Users can tap on a notification and interact with it

Notifications are the most widely used functionality in Android, from SMS and missed calls to wifi network connection and Twitter notifications. They are used everywhere. 

## Broadcast Receivers

Broadcast receiver is a way to listen to systemwide events happening in your phone or tablet. Using Broadcast Receivers, we can create some great applications like Call Number finder, SMS blocker etc,  which works when some events happen on your device. 
...
We can also generate a custom broadcast from inside our application. Whenever you want to create any application which works with system events use a broadcast receiver. 
...
We are going to demonstrate all the above components in the official Twitter app. Below is the screenshot of the app's screens with relevant components highlighted:
...
<div class="row-fluid">
 <div class="span8">
  <div style="margin:auto">
	<p>You clicked on the blue Twitter icon to launch the Twitter app. This icon is called a Launcher icon.</p>
...
<div class="row-fluid">
 <div class="span8">
  <div style="margin:auto">
    <p>Once you key in your login details & hit the 'Sign In' button, Twitter app does a network call to verify your username & password. This is done using <b>AsyncTask</b>. AsyncTasks are the Android equivalent of Threads meant for short-term one-off operations (like HTTP network calls).  
	<p>After the username and password are verified you see the <b>new Activity</b> which shows your Twitter stream. This is done using <b>Intent</b> that helps you to move from one activity to another. 
  </div>
 </div>
 <div class="span4">
...
<div class="row-fluid">
 <div class="span8">
  <div style="margin:auto">
   <p>At the top of the screen is the <b>Action Bar</b>. The <b>Action Bar</b> is an example of a <b>User Interface</b>. 
   <p>The list of tweets is a <b>ListView</b> object which is a <b>View</b> element. You can click on a tweet to go to the tweet detail screen which is again a <b>new Activity</b>. The navigation happens using an <b>Intent</b>. Notice that the tweet detail screen looks the same except the content is changing. The content is passed through the Intent when you click on the tweet.
...
<div class="row-fluid">
 <div class="span8">
  <div style="margin:auto">
    ...
    <p>While this Service is activated, the Activity keeps showing a 'Loading' sign. The tweets are locally stored in the Twitter app. The Service is supposed to update the tweets in the SQLite database. A Broadcast Receiver is set up to listen for an event which is triggered when the Service ends. The Broadcast Receiver then updates the tweet list & hides the 'Loading' sign. 
   </div>
 </div>
 <div class="span4">
...
Once loaded, the tweets are accessible even when there is no network connection. As mentioned earlier, the tweets are stored in the database. Otherwise, it would not be possible to show the tweets when the app is offline.

Typically, a database is never directly accessed. Instead, the Twitter app may use a Content Provider. Content Providers help to provide abstraction over how data is stored and accessed in an application. 
...
<div class="row-fluid">
 <div class="span8">
  <div style="margin:auto">
    ...
    <p>Services are one way to receive notifications but to prevent an app from draining battery the Twitter app uses a Push Notification. Push notifications are a way to push data from an application even if application is not running. 
   </div>
 </div>
 <div class="span4">
